...
/***/ (function(module, exports, __webpack_require__) {
	...
	var Row = React.createClass({
	  ...
	  getCell: function getCell(column, i, selectedColumn) {
	    ...
	    var keyValue = subKey !== undefined ? "${this.key}-${this.subKey}-${this.idx}" : "${this.key}-${this.idx}";
	    var column_copy = JSON.parse(JSON.stringify(column));
	    column_copy.idx = idx;
	    var keyFormatter = new Function('return `' + keyValue + '`;').call(column_copy);
	    var baseCellProps = { key: keyFormatter, idx: i, rowIdx: idx, height: this.getRowHeight(), column: column, cellMetaData: cellMetaData };
	  ...
	  getCellValue: function getCellValue(key, subKey) {
	    var val = void 0;
	    if (key === 'select-row') {
